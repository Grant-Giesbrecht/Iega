...
/*Creates a string from the double. The function automatically selects whether to use
 scientific or fixed notations. Additionally, the function removes extraneous characters
 such as trailing zeros on decimals, leading zeros on powers, and '+' signs.
 
 input - the double to convert to a string
 precision - number of decimal places to display, however trailing zeros will
 	be eliminated
 threshold - power of 10 at which to switch from fixed to scientific notation. Looks
 	at absolute value only, upper bound (eg. 10^3) is inclusive, lower (eg. 10^-3)
 	is exclusive.
 
 Returns the formatted string
*/
std::string dtos(double input, double precision, double threshold){
	
	stringstream ss;
	
	//Select format (sci vs fix)
	char format = 'f';
	if (abs(input) >= pow(10, threshold) || abs(input) < pow(10, -1*threshold)){
		format = 's';
	}
	
	//Set format
	if (format == 'f'){ //Fixed
		ss.setf(std::ios::fixed, std::ios::floatfield); //set SS to fixed notation
	}else{ //Scientific
		ss.setf(std::ios::scientific, std::ios::floatfield); //set SS to scientific notation
	}
	...
	//Create string
	ss << input;
	string frmtstr = ss.str();
	
	//Edit default format to be more concise
	if (format == 'f'){
		
		//remove trailing zeros (and decimal if no zeros)
		if (frmtstr.find('.') == std::string::npos){ //Return now if no decimal exists - this prevents eliminating significant zeros
			return frmtstr;
		}
		char lastchar = frmtstr[frmtstr.length()-1];
		while (lastchar == '0' || lastchar == '.'){
			frmtstr = frmtstr.substr(0, frmtstr.length()-1); //Eliminate last character
			if (lastchar == '.'){ // Quit loop if arrived at decimal
				break;
			}
			lastchar = frmtstr[frmtstr.length()-1]; //Update 'lastchar'
		}
		
	}else{
		
		//Find the 'e' character
		size_t eidx = frmtstr.find('e');
		if (eidx == std::string::npos){
			eidx = frmtstr.find('E');
			if (eidx == std::string::npos){
				return frmtstr; //Return early if 'e' can't be found (upper or lowercase). Something went wrong
			}
		}
		
		//Remove extra signs and zeros on power
		char nextchar;
		size_t padding = 0;
		while (eidx+1 < frmtstr.length()-1){
			nextchar = frmtstr[eidx+1+padding];
			if (nextchar != '+' && nextchar != '-' && nextchar != '0'){ //Quit loop when extraneous characters are eliminated (ie. non +,-,0 is found)
				break;
			}
			if (nextchar == '+' || nextchar == '0'){
				frmtstr.erase(eidx+1, 1); //Erase extraneous character
			}else{
				padding++;
				if (padding > 2){ //If padding is > 2, that means 2+ '-' signs are in the power. Something went wrong
					return frmtstr;
				}
			}
		...
		char lastchar = frmtstr[eidx-1];
		while (lastchar == '0' || lastchar == '.'){
			frmtstr.erase(eidx-1, 1); //Eliminate extraneous character
			if (lastchar == '.'){ // Quit loop if arrived at decimal
				break;
			}
			eidx--; //Update 'eidx'
			lastchar = frmtstr[eidx-1]; //Update 'lastchar'
		}
...
/*
 Reads a string and returns a boolean
 
 input - string to convert
 
 Returns boolean value of input. Returns false if unrecognized.
 */
bool str_to_bool(std::string input){
	std::string inputuc = to_uppercase(input);
	
	if (inputuc == "TRUE" || inputuc == "T"  || inputuc == "1"){
		return true;
	}
