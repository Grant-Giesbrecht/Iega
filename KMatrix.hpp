...
public:
	...
	std::vector<T> get_rowv(size_t row) const;
...
template <class T>
KMatrix<T> operator+(KMatrix<T> lv, const T& rv);
...
template <class T>
KMatrix<T> abs(const KMatrix<T>& powers);
	...
	while (last_value < end){
		...
		for (double i = 1 ; i <= num_per_decade ; i++){
			...
			if (last_value >= start && last_value <= end ){
				temp_vals.push_back(last_value);
			}else if(last_value > end){
				break;
			}
...
KMatrix<T> pow(const KMatrix<T>& a, double power){
	...
	for (size_t r = 0 ; r < a.rows() ; r++){
		for (size_t c = 0 ; c < a.cols() ; c++){
			...
			if (isnan(out(r,c))){
				std::cout << "POWER NAN: " << a.get(r, c) << "^" << power << std::endl;
			}
		}
	}
...
KMatrix<T> pow(double value, const KMatrix<T>& powers){
	...
	for (size_t r = 0 ; r < powers.rows() ; r++){
		for (size_t c = 0 ; c < powers.cols() ; c++){
			...
			if (isnan(out(r,c))){
				std::cout << "POWER NAN: " << value << "^" << powers.get(r, c) << std::endl;
			}
		}
	}
