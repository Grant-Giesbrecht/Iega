...
#define MATRIX_D_MARK "m<d>"
#define MATRIX_S_MARK "m<s>"
#define MATRIX_B_MARK "m<b>"
...
/*
 Returns a vector containing all the variable's IDs (names)
...
/*
 Returns a vector containing all the variable's IDs (names) of the type 'type'
 
 type - type of variables to list
	 d - double
	 D - matrix<double>
	 S - matrix<string>
	 B - matrix<bool>
 ...
 returns the vector.
 */
vector<string> KVar::get_variable_type_IDs(char type){
	vector<string> out;
	for (int i = 0 ; i < KVar::variables.size() ; i++){
		if (KVar::variables[i].type == type) out.push_back(KVar::variables[i].ID);
	}
	return out;
}
...
bool KVar::add_variable(string var_id, KMatrix<double> val, string comment){
...
bool KVar::add_variable(string var_id, KMatrix<std::string> val, string comment){
...
bool KVar::add_variable(string var_id, KMatrix<bool> val, string comment){
...
bool KVar::set_variable(string var_id, KMatrix<double> val){
...
bool KVar::set_variable(string var_id, KMatrix<std::string> val){
...
bool KVar::set_variable(string var_id, KMatrix<bool> val){
...
bool KVar::get_variable(string var_id, KMatrix<double>& val){
...
bool KVar::get_variable(string var_id, KMatrix<std::string>& val){
...
bool KVar::get_variable(string var_id, KMatrix<bool>& val){
...
bool KVar::add_matrix(std::string var_id, KMatrix<double>& val, string comment){

	return add_variable(var_id, val, comment);
}

bool KVar::add_matrix(std::string var_id, KMatrix<std::string>& val, string comment){
	
	return add_variable(var_id, val, comment);
}

bool KVar::add_matrix(std::string var_id, KMatrix<bool>& val, string comment){
	
	return add_variable(var_id, val, comment);
}

bool KVar::get_matrix(std::string var_id, KMatrix<double>& out){

	return get_variable(var_id, out);
    
}

bool KVar::get_matrix(std::string var_id, KMatrix<std::string>& out){
	
	return get_variable(var_id, out);
	
}

bool KVar::get_matrix(std::string var_id, KMatrix<bool>& out){
	
	return get_variable(var_id, out);
	
}

bool KVar::set_matrix(std::string var_id, KMatrix<double> in){
    
    return set_variable(var_id, in);
    
}

bool KVar::set_matrix(std::string var_id, KMatrix<std::string> in){
	
	return set_variable(var_id, in);
	
}

bool KVar::set_matrix(std::string var_id, KMatrix<bool> in){
...
/*
 Returns a vetor of strings containing all of the strings in the KVar object.
 */
std::vector<std::string> KVar::getStrings(){
	vector<string> out;
	for (int i = 0 ; i < KVar::variables.size() ; i++){
		if (KVar::variables[i].type == 's'){
			out.push_back(KVar::variables[i].s);
		}
	}
	return out;
}
...
/*
 Returns a vetor of strings containing all of the strings in the KVar object.
 */
void KVar::getStringsWithNames(std::vector<std::string>& contents, std::vector<std::string>& names){
	
	for (int i = 0 ; i < KVar::variables.size() ; i++){
		if (KVar::variables[i].type == 's'){
			contents.push_back(KVar::variables[i].s);
			names.push_back(KVar::variables[i].ID);
		}
	}
...
bool KVar::load_KV1(std::string filename, bool header_only){
    long temp;
    return load_KV1(filename, temp, header_only);
}
...
/*
 Loads a KV1 file ('filename').
 
 filename - name of KV1 file to read
 fail_line - (if read fails) line at which read failed. -1 if file does not exist.
 	-2 if an error with the version specifier is detected. -3 if a header error occurs.
 
 Returns true if file read successfully, else false
 */
bool KVar::load_KV1(string filename, long& fail_line, bool header_only){
...
//        words = parse(line, " "); //Parse
        ...
        if(words[0] == "#HEADER"){
        ...
        }else if (words[0] == DOUBLE_MARK){ //Read double
            ...
            if (words.size() > 4){
                ...
                if (read_comments){
                    comment = cat_tokens(words, 5, " "); //Save comment
                }
            }
        ...
        }else if(words[0] == MATRIX_D_MARK){ //Read matrix of doubles
		...
		}else if(words[0] == MATRIX_S_MARK){ //Read matrix of strings
			
			string var_name;
			KMatrix<std::string> val;
			string comment;
		...
		}else if(words[0] == MATRIX_B_MARK){ //Read matrix of doubles
        ...
        }else if(words[0] == STRING_MARK){ //Read string
        ...
        }else if(words[0] == BOOL_MARK){ //Read bools
    ...
    for (int i = 0 ; i < variables.size() ; i++){
        switch (variables[i].type) {
            case 'd':
                ...
                break;
            case 'D':
                file << MATRIX_D_MARK << " " << variables[i].ID << " " << variables[i].kmd.to_string("[") << ";";
                if (variables[i].comment.length() > 0 && KVar::save_comments){
                    file << " //" << variables[i].comment;
                }
                file << endl;
                break;
			case 'S':
				file << MATRIX_S_MARK << " " << variables[i].ID << " " << variables[i].kms.to_string("[\"") << ";";
				if (variables[i].comment.length() > 0 && KVar::save_comments){
					file << " //" << variables[i].comment;
				}
				file << endl;
				break;
			case 'B':
				file << MATRIX_B_MARK << " " << variables[i].ID << " " << variables[i].kmb.to_string("[u") << ";";
				if (variables[i].comment.length() > 0 && KVar::save_comments){
					file << " //" << variables[i].comment;
				}
				file << endl;
...
//                break;
//            case 'm':
//                file << "matrix " << variables[i].ID << " = " << variables[i].km.to_string() << ";";
//                if (variables[i].comment.length() > 0 && KVar::save_comments){
//                    file << " //" << variables[i].comment;
//                }
//                file << endl;
...
void KVar::print(int indent, bool use_spaces){
	...
	for (int i = 0 ; i < variables.size() ; i++){
		if (variables[i].type == 'D'){
            ...
            if (print_comments && variables[i].comment.length() > 0) cout << comment_string << variables[i].comment;
            cout << endl;
			KMatrix<double> km(variables[i].kmd);
			int mindent = indent;
			if (!use_spaces) mindent *= 8;
			mindent += 12;
			cout << indent_line(mindent, true) << km.to_string("[") << endl;
			counter++; //Increment counter
		}
		if (variables[i].type == 'S'){
			...
			if (print_comments && variables[i].comment.length() > 0) cout << comment_string << variables[i].comment;
			cout << endl;
			KMatrix<string> km(variables[i].kms);
			int mindent = indent;
			if (!use_spaces) mindent *= 8;
			mindent += 12;
			cout << indent_line(mindent, true) << km.to_string("[\"") << endl;
		...
		if (variables[i].type == 'B'){
			...
			if (print_comments && variables[i].comment.length() > 0) cout << comment_string << variables[i].comment;
			cout << endl;
			KMatrix<bool> km(variables[i].kmb);
			int mindent = indent;
			if (!use_spaces) mindent *= 8;
			mindent += 12;
			cout << indent_line(mindent, true) << km.to_string("[u") << endl;
			counter++; //Increment counter
		}
	}
